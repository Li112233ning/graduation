...
export function useHook() {
  const searchFormParams = reactive<BannerQuery>({
    title: "",
    status: undefined,
    pageNum: 1,
    pageSize: 10
  ...
  const selectedRows = ref<BannerDTO[]>([]);
  ...
  const columns: TableColumnList = [
    ...
    },
    {
      ...
      cellRenderer: ({ row }) => (
        <el-image
          src={row.imageUrl || "https://picsum.photos/300/150"}
          fit="cover"
          style="width: 120px; height: 60px; border-radius: 4px"
          preview-src-list={[row.imageUrl || "https://picsum.photos/1200/600"]}
        />
      )
    },
    {
  ...
  async function getList() {
    ...
    try {
      ...
      const response = await getBannerList(searchFormParams);
  ...
  function handleSelectionChange(val: BannerDTO[]) {
    selectedRows.value = val;
  }

  function openDialog(row?: BannerDTO) {
    addDialog({
      title: row ? "编辑轮播图" : "新增轮播图",
      props: {
        formInline: {
          bannerId: row?.bannerId ?? undefined,
          title: row?.title ?? "",
      ...
      beforeSure: (done, { options }) => {
        ...
        function chores() {
          message(
            `您${row ? "编辑" : "新增"}了轮播图${curData.title}的数据`,
            { type: "success" }
          );
          done();
          getList();
  ...
  async function handleDelete(row: BannerDTO) {
    try {
  ...
  function handleBatchDelete() {
    ...
    ElMessageBox.confirm(
      ...
      .then(async () => {
        const ids = selectedRows.value.map(row => row.bannerId);
