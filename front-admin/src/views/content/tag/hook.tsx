...
export function useHook() {
  ...
  async function getList() {
    ...
    try {
      ...
      const { data } = await getTagList(searchFormParams);
      dataList.value = data.rows;
      pagination.total = data.total;
    } catch (error) {
      console.error("获取标签列表失败:", error);
      message("获取标签列表失败", { type: "error" });
    } finally {
  ...
  function handleSelectionChange(val: TagDTO[]) {
    selectedRows.value = val;
  }

  function openDialog(row?: TagDTO) {
    addDialog({
      title: row ? "编辑标签" : "新增标签",
      props: {
        formInline: {
      ...
      beforeSure: (done, { options }) => {
        ...
        function chores() {
          message(`您${row ? "编辑" : "新增"}了标签名称为${curData.tagName}的数据`, {
            type: "success"
          });
          done();
          getList();
  ...
  async function handleDelete(row: TagDTO) {
    try {
      await deleteTag(row.tagId);
      message(`删除标签${row.tagName}成功`, { type: "success" });
      getList();
    } catch (error) {
  ...
  function handleBatchDelete() {
    ...
    ElMessageBox.confirm(
      ...
      .then(async () => {
        const ids = selectedRows.value.map(row => row.tagId);
